...
const SYSTEM_PROMPT = `You are a senior UK planning consultant with expert knowledge of the Town and Country Planning (General Permitted Development) (England) Order 2015 (GPDO) as amended, the Town and Country Planning Act 1990, the Planning Policy Framework (NPPF), and local authority planning policies across England.
...
function buildUserPrompt(form: FormState) {
  const projectLabel =
    ...
    form.projectType;
  const propertyLabel =
    PROPERTY_TYPES.find((p) => p.id === form.propertyType)?.label ||
    form.propertyType;
...
interface PriorApprovalDetails {
  required: boolean;
  process: string;
  neighbourConsultation: boolean;
  consultationPeriod: string | null;
  fee: string;
  determination: string;
}

interface CilAssessment {
  liable: boolean | "unknown";
  reason: string;
  selfBuildExemption: boolean;
  exemptionFormRequired: boolean;
}

interface PreAppAdvice {
  recommended: boolean;
  reason: string;
  typicalCost: string;
}
...
interface Report {
  ...
  pdClass: string | null;
  keyRules: string[];
  priorApprovalDetails: PriorApprovalDetails | null;
  cilAssessment: CilAssessment | null;
  planningStatement: string;
  riskFlags: RiskFlag[];
  nextSteps: string[];
  preAppAdvice: PreAppAdvice | null;
  disclaimer: string;
}
...
const verdictConfig = {
  ...
  planning_permission: {
    label: "Planning Permission Required",
    color: "#2563eb",
    bg: "#dbeafe",
    icon: "i",
  },
  likely_refused: {
    label: "Likely to be Refused",
    color: "#dc2626",
    bg: "#fee2e2",
    icon: "✗",
  },
...
export default function PlanningApp() {
  ...
  // Check for paid=true and generate report automatically
  useEffect(() => {
    if (typeof window === "undefined") return;
    const paid = searchParams.get("paid");
    if (paid === "true") {
      const savedForm = sessionStorage.getItem("planningForm");
      if (savedForm) {
        ...
        // Generate report with saved form
        generateReport(parsedForm);
      }
    }
  }, [searchParams]);

  const generateReport = async (formData: FormState) => {
    setLoading(true);
    setError(null);
    try {
      ...
      setReport(parsed);
      setStep(4);
      // Clear saved form after successful generation
  ...
  const handleCheckout = async () => {
    // Save form to sessionStorage before redirecting
    if (typeof window !== "undefined") {
  ...
  const vc = report
    ? verdictConfig[report.verdict] || verdictConfig.planning_permission
    : null;
  ...
  return (
    ...
    >
      ...
      <div className="pa-wrap">
        ...
        <p className="pa-sub">
          Instant UK planning guidance — powered by AI, grounded in the GPDO
        </p>
        ...
        {step < 4 && (
          <div className="pa-steps">
            {STEPS.map((s, i) => (
              <div
                key={s}
                className={`pa-step ${i === step ? "active" : i < step ? "done" : ""}`}
              >
                {s}
              </div>
            ))}
          ...
          <div className="pa-card">
            ...
            <div className="pa-grid" style={{ marginBottom: "1.25rem" }}>
              {PROJECT_TYPES.map((p) => (
                <button
                  key={p.id}
                  className={`pa-option ${form.projectType === p.id ? "sel" : ""}`}
            ...
            <textarea
              ...
              placeholder="e.g. Single-storey rear extension, 4m deep, flat roof, to create open-plan kitchen-diner..."
          ...
          <div className="pa-card">
            ...
            <div className="pa-grid" style={{ marginBottom: "1.25rem" }}>
              {PROPERTY_TYPES.map((p) => (
                <button
                  key={p.id}
                  className={`pa-option ${form.propertyType === p.id ? "sel" : ""}`}
            ...
            <label className="pa-label">
              Dimensions (metres) — fill in what you know
            </label>
            <div className="pa-row3" style={{ marginBottom: "1.25rem" }}>
              <div>
                <label className="pa-label" style={{ fontSize: ".75rem" }}>
                  Depth
                </label>
                <input
                  className="pa-input"
            ...
            <div className="pa-grid">
              {(
                ...
                <label
                  key={key}
                  className={`pa-check ${form[key] ? "sel" : ""}`}
                >
                  <input
                    type="checkbox"
                    checked={form[key]}
            ...
            <div className="pa-navrow">
              <button className="pa-btn-ghost" onClick={() => setStep(0)}>
                ← Back
              </button>
              <button
                className="pa-btn"
        ...
        {step === 2 && (
          <div className="pa-card">
            ...
            <input
              className="pa-input"
              style={{ marginBottom: "1.5rem", maxWidth: 200 }}
            ...
            >
              <strong>Why we need this:</strong> Planning rules vary
              significantly by location. London boroughs frequently have Article
              4 directions removing standard permitted development rights.
              Conservation area boundaries and AONB designations are
              location-specific.
            </div>
            <div className="pa-navrow">
              <button className="pa-btn-ghost" onClick={() => setStep(1)}>
                ← Back
              </button>
              <button
                className="pa-btn"
        ...
        {step === 3 && (
          <div className="pa-card">
            ...
            {(
              [
                ...
                [
                  "Dimensions",
                  [
                    form.depthM && `${form.depthM}m deep`,
                    form.widthM && `${form.widthM}m wide`,
                    form.heightM && `${form.heightM}m high`,
                  ]
                    .filter(Boolean)
                    .join(" · ") || "Not specified",
                ],
                [
                  "Constraints",
                  [
                    form.conservationArea && "Conservation area",
                    form.listedBuilding && "Listed building",
                    form.aonb && "AONB",
                    form.previousExtensions && "Prior extensions",
                  ]
                    .filter(Boolean)
                    .join(", ") || "None flagged",
                ],
              ...
              >
                <span style={{ width: 110, color: "#888", flexShrink: 0 }}>
                  {k}
                </span>
            ...
            <div className="pa-navrow">
              <button className="pa-btn-ghost" onClick={() => setStep(2)}>
                ← Back
              </button>
              <button className="pa-btn" onClick={handleCheckout} disabled={loading}>
                {loading ? "Processing…" : "Get My Report — £19"}
              </button>
            </div>
          </div>
        )}
        ...
        {loading && (
          <div className="pa-loading">
            ...
            <p>
              Checking GPDO rules, prior approval thresholds, CIL liability, and
          ...
          <>
            <div className="pa-verdict" style={{ background: vc?.bg }}>
              ...
              <div className="pa-verdict-body">
                ...
                {report.pdClass && (
                  <p className="pa-conf">
                    GPDO {report.pdClass} · Confidence: {report.confidence}
                  </p>
                )}
            ...
            <div className="pa-tabs">
              {(
                ...
                <div
                  key={id}
                  className={`pa-tab ${activeTab === id ? "active" : ""}`}
            ...
            <div className="pa-card">
              {activeTab === "summary" && (
                <>
                  <p className="pa-section-title">
                    Key planning rules that apply
                  </p>
                  <ul className="pa-rules" style={{ paddingLeft: "1.2rem" }}>
                    {report.keyRules?.map((r, i) => (
                      <li key={i}>{r}</li>
                    ))}
                  </ul>
                  {report.preAppAdvice && (
                    <div
                      style={{
                        marginTop: "1.25rem",
                        background: report.preAppAdvice.recommended
                          ? "#fffbeb"
                          : "#f0fdf4",
                        border: `1px solid ${report.preAppAdvice.recommended ? "#fde68a" : "#bbf7d0"}`,
                        borderRadius: 6,
                        padding: ".9rem 1rem",
                      }}
                    >
                      <p
                        style={{
                          ...
                          letterSpacing: ".06em",
                          color: report.preAppAdvice.recommended
                            ? "#92400e"
                            : "#166534",
                        }}
                      >
                      ...
                      >
                        {report.preAppAdvice.reason}
                      </p>
                      {report.preAppAdvice.recommended && (
                        <p
                          style={{
                            margin: 0,
                            fontSize: ".82rem",
                        ...
                        >
                          Typical cost: {report.preAppAdvice.typicalCost}
                        </p>
                      )}
                    </div>
                  )}
                ...
                <>
                  ...
                  {report.priorApprovalDetails ? (
                    report.priorApprovalDetails.required ? (
                      <>
                        ...
                        >
                          ...
                          >
                            Prior approval required for this project
                          </p>
                          <p
                            style={{
                              margin: 0,
                          ...
                          >
                            {report.priorApprovalDetails.process}
                          </p>
                        </div>
                        {(
                          [
                            [
                              ...
                              report.priorApprovalDetails.neighbourConsultation
                                ? "Yes — adjoining owners/occupiers will be notified by the LPA"
                                : "Not required for this application type",
                            ],
                            [
                              "Consultation period",
                              report.priorApprovalDetails.consultationPeriod ||
                                "21 days from notification",
                            ],
                            [
                              "Application fee",
                              report.priorApprovalDetails.fee ||
                                "£120 (householder)",
                            ],
                            [
                              "LPA determination period",
                              report.priorApprovalDetails.determination ||
                                "42 days from valid application",
                            ],
                          ] as const
                        ).map(([k, v]) => (
                          <div
                        ...
                        >
                          <strong>Important:</strong> If the LPA receives no
                          objection from neighbours and the proposal meets all
                          Class A conditions, prior approval must be granted. If
                          the LPA does not determine the application within 42
                          days of a valid submission, prior approval is{" "}
                          <em>deemed granted</em>.
                        </div>
                      </>
                    ) : (
                      <div
                      ...
                      >
                        ...
                        >
                          Prior approval is <strong>not required</strong> for
                          this project. Your proposal either falls within the
                          standard permitted development limits or requires full
                          planning permission — see the Key Rules tab for
                          details.
                        </p>
                      </div>
                    )
                    ...
                    <p style={{ color: "#888", fontSize: ".9rem" }}>
                      Prior approval information not applicable to this project
                      type.
                    </p>
                  )}
                </>
                  ...
                  {report.cilAssessment ? (
                    <>
                      <div
                        style={{
                          background:
                            report.cilAssessment.liable === true
                              ? "#fff7ed"
                              : report.cilAssessment.liable === false
                                ? "#f0fdf4"
                                : "#f8f8f8",
                      ...
                      {report.cilAssessment.selfBuildExemption && (
                        ...
                        >
                          ...
                          >
                            Self-build CIL exemption available
                          </p>
                          <p
                            style={{
                              margin: 0,
                          ...
                          >
                            You may be eligible to claim a self-build exemption
                            from CIL.
                            {report.cilAssessment.exemptionFormRequired && (
                              <>
                                {" "}
                                You{" "}
                                <strong>
                                  must submit Form 7 (Self Build Exemption
                                  Claim) BEFORE commencing any works
                                </strong>
                                . If you start works without claiming, the
                                exemption is lost and full CIL becomes
                                immediately payable.
                              </>
                            )}
                          </p>
                      ...
                      >
                        Not all councils have adopted a CIL charging schedule.
                        Check your council&apos;s planning portal for their
                        current CIL charging schedule and any exemptions that
                        may apply to your project.
                      </div>
                    </>
                  ) : (
                    <p style={{ color: "#888", fontSize: ".9rem" }}>
                      CIL assessment not available for this project.
                    </p>
                  )}
                </>
              )}
              ...
              {activeTab === "statement" && (
                <>
                  ...
                  >
                    Suitable for submission to your Local Planning Authority ·
                    Cites NPPF policy
                  </p>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      a.download = "planning-statement.txt";
                      a.click();
                    }}
                  >
                    Download Statement
                  ...
                  {report.riskFlags?.length > 0 ? (
                    report.riskFlags.map((r, i) => (
                      <div key={i} className="pa-risk">
                        <div
                          className="pa-risk-dot"
                          style={{ background: riskColor[r.level] || "#888" }}
                        />
                        <div>
                          <span
                            className="pa-badge"
